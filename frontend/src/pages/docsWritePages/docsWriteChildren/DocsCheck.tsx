...
export const DocsCheck = ({ curTemplate, role }: { curTemplate: string, role: string }) => {
  ...
  const share = async () => {
    ...
    const signatureURL = (role == "채권자" ? creditor_signature : debtor_signature)
    ...
    return (
        <div className="h-full flex flex-col gap-y-6 overflow-hidden">
            ...
            {previousPage == "/docs/detail/G1/signature" ?
              <atoms.LongButton onClick={share} className="mb-20" children="저장 후 공유" colorType="black" />
              :
              <Link to={getNextPage()}>
