...
import com.ssafy.ddukdoc.global.aop.AuthenticatedAccess;
...
public class UserController {
    ...
    @GetMapping("/status/general")
    @GeneralAccess
    @Operation(summary = "일반 로그인 상태 확인", description = "로그인 상태를 확인합니다. 없으면 401")
    @ApiErrorCodeExamples({ErrorCode.UNAUTHORIZED_ACCESS})
    public ResponseEntity<CommonResponse<Void>> checkGeneralLoginStatus() {
        return CommonResponse.ok();
    }

    @GetMapping("/status/ssafy")
    @SSAFYAccess
    @Operation(summary = "싸피 로그인 상태 확인", description = "로그인 상태를 확인합니다. 없으면 401")
    @ApiErrorCodeExamples({ErrorCode.UNAUTHORIZED_ACCESS})
    public ResponseEntity<CommonResponse<Void>> checkSsafyLoginStatus() {
        return CommonResponse.ok();
    }
}